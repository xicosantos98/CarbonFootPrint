...
router
    ...
    .get('/', function (req, res) {
        ...
        for (var i = 1; i <= count; i++) {
            ...
            organizations.push({
                id: org[0].toNumber(), CO2eq: co2eq, name: org[3], products: prods,
                monthly_activities: m_activities, monthly_fix_costs: fix_costs
            })
        }
    ...
    .get('/:id', function (req, res) {
        ...
        if (req.params.id == null) {
            ...
            if (organization[0].toNumber() == 0) {
                ...
                var co2eq = (organization[1] * Math.pow(10, -(organization[2]))).toFixed(organization[2])
                var prods = c_instance.getOrgProducts(organization[0].toNumber());
                var m_activities = c_instance.getOrgMactivities(organization[0].toNumber());
                var fix_costs = c_instance.getOrgFixCosts(organization[0].toNumber());

                res.status(200).json({
                    id: organization[0].toNumber(), CO2eq: co2eq, name: organization[3], products: prods,
                    monthly_activities: m_activities, monthly_fix_costs: fix_costs
                })
            }
    ...
    .post('/', function (req, res) {
        ...
        if (req.body.name == null) {
            ...
            try {
                c_instance.addOrganization(req.body.name, [], [], [], { from: req.headers.address, gas: 3000000 });
